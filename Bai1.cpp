...
void getData(int &V, int &src, int &dest, int** &G){
	...
	ifstream graph("D:\\C++\\CauTrucDuLieu&GiaiThuatNangCao\\Cac phuong phap sap xep co ban\\graph\\Bai1.txt", ios::in);
	if(!graph.is_open()){
...
// ham tim dinh có do dài duong di nho nhat chua duac duyet
...
void dijkstra(int src, int V, int** G, int dest){
    ...
    display(dist, par, src, dest);  // hien thi ket qua dinh dau va dinh cuoi
}
int main(){
